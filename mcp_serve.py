"""
Drewgent MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Drewgent-specific extra)

Usage:
    drewgent mcp serve
    drewgent mcp serve --verbose
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using DREW_HOME."""
    try:
        from drewgent_constants import get_drewgent_home
        return get_drewgent_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("DREW_HOME", Path.home() / ".drewgent")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from drewgent_constants import get_drewgent_home
        directory_file = get_drewgent_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("DREW_HOME", Path.home() / ".drewgent")
...
class EventBridge:
    ...
    This is the Drewgent equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from drewgent_constants import get_drewgent_home
            db_file = get_drewgent_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("DREW_HOME", Path.home() / ".drewgent")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Drewgent MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'drewgent-agent[mcp]'"
        )

    mcp = FastMCP(
        "drewgent",
        instructions=(
            "Drewgent Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Drewgent MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
