"""
Tests for mcp_serve — Drewgent MCP server.
...
def _isolate_drewgent_home(tmp_path, monkeypatch):
    """Redirect DREW_HOME to a temp directory."""
    monkeypatch.setenv("DREW_HOME", str(tmp_path))
    try:
        import drewgent_constants
        monkeypatch.setattr(drewgent_constants, "get_drewgent_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking drewgent_state schema."""
