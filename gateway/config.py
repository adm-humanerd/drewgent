...
def _resolve_streaming_config(data: Dict[str, Any]) -> "StreamingConfig":
    """Resolve streaming config from multiple possible locations.

    Checks in order of precedence:
    1. Top-level 'streaming' key (dict or bool)
    2. 'display.streaming' key (dict or bool, legacy location)

    Returns StreamingConfig with appropriate defaults.
    """
    # Get top-level streaming config
    top_stream = data.get("streaming")

    # Get display.streaming (legacy location)
    display_data = data.get("display")
    display_stream = (
        display_data.get("streaming") if isinstance(display_data, dict) else None
    )

    # Normalize each to dict format
    ...
    top_dict = _normalize(top_stream)
    display_dict = _normalize(display_stream)

    # Merge: top-level takes precedence over display
    merged = display_dict.copy()
    merged.update(top_dict)

    return StreamingConfig.from_dict(merged)
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_drewgent_home() / "sessions")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        sessions_dir = get_drewgent_home() / "sessions"
        ...
        return cls(
            ...
            streaming=_resolve_streaming_config(data),
        )
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.drewgent/config.yaml (primary user-facing config)
    3. ~/.drewgent/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_drewgent_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if isinstance(streaming_cfg, dict):
                ...
                # Handle boolean streaming: true/false
                gw_data["streaming"] = {"enabled": bool(streaming_cfg)}

            # Also check display.streaming (legacy location)
            display_cfg = yaml_cfg.get("display", {})
            if isinstance(display_cfg, dict):
                display_streaming = display_cfg.get("streaming")
                if display_streaming is not None and "streaming" not in gw_data:
                    if isinstance(display_streaming, dict):
                        gw_data["streaming"] = display_streaming
                    else:
                        gw_data["streaming"] = {"enabled": bool(display_streaming)}
