...
The PID file lives at ``{DREW_HOME}/gateway.pid``.  DREW_HOME defaults to
``~/.drewgent`` but can be overridden via the environment variable.  This means
separate DREW_HOME directories naturally get separate PID files — a property
...
_GATEWAY_KIND = "drewgent-gateway"
_RUNTIME_STATUS_FILE = "gateway_state.json"
_LOCKS_DIRNAME = "gateway-locks"


def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting DREW_HOME."""
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Drewgent gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different DREW_HOME dirs).
    """
