"""
Unified tool configuration for Drewgent Agent.

`drewgent tools` and `drewgent setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.drewgent/config.yaml under
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from drewgent_cli.plugins import discover_plugins, get_plugin_toolsets
...
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "drewgent-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "drewgent-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "drewgent-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "drewgent-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "drewgent-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "drewgent-signal"},
    "homeassistant": {"label": "🏠 Home Assistant", "default_toolset": "drewgent-homeassistant"},
    "email":    {"label": "📧 Email",      "default_toolset": "drewgent-email"},
    "matrix":   {"label": "💬 Matrix",     "default_toolset": "drewgent-matrix"},
 "dingtalk": {"label": "💬 DingTalk", "default_toolset": "drewgent-dingtalk"},
    "feishu": {"label": "🪽 Feishu", "default_toolset": "drewgent-feishu"},
    "wecom": {"label": "💬 WeCom", "default_toolset": "drewgent-wecom"},
    "api_server": {"label": "🌐 API Server", "default_toolset": "drewgent-api-server"},
    "mattermost": {"label": "💬 Mattermost", "default_toolset": "drewgent-mattermost"},
    "webhook": {"label": "🔗 Webhook", "default_toolset": "drewgent-webhook"},
}
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_drewgent_home()}/drewgent-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in drewgent-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "drewgent-cli" (which include all _HERMES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "drewgent-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `drewgent tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by drewgent tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (drewgent-cli, drewgent-telegram, etc.)
    ...
    if not env_vars:
        ...
        if managed_feature:
            ...
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.drewgent/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `drewgent tools` and `drewgent setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Drewgent Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://drewgent-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from drewgent_constants import display_drewgent_home
    print(color(f"  Tool configuration saved to {display_drewgent_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'drewgent' or gateway restart.", Colors.DIM))
    print()
