"""
Gateway subcommand for drewgent CLI.

Handles: drewgent gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from drewgent_cli.config import get_env_value, get_drewgent_home, save_env_value, is_managed, managed_error
# display_drewgent_home is imported lazily at call sites to avoid ImportError
# when drewgent_constants is cached from a pre-update version during `drewgent update`.
from drewgent_cli.setup import (
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "drewgent-gateway*",
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (DREW_HOME-scoped).
...
_SERVICE_BASE = "drewgent-gateway"
SERVICE_DESCRIPTION = "Drewgent Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current DREW_HOME.

    Returns ``""`` for the default ``~/.drewgent``, the profile name for
    ``~/.drewgent/profiles/<name>``, or a short hash for any other custom
    DREW_HOME path.
    """
    import hashlib
    import re
    from pathlib import Path as _Path
    home = get_drewgent_home().resolve()
    default = (_Path.home() / ".drewgent").resolve()
    if home == default:
        return ""
    # Detect ~/.drewgent/profiles/<name> pattern → use the profile name
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary DREW_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def get_service_name() -> str:
    """Derive a systemd service name scoped to this DREW_HOME.

    Default ``~/.drewgent`` returns ``drewgent-gateway`` (backward compatible).
    Profile ``~/.drewgent/profiles/coder`` returns ``drewgent-gateway-coder``.
    Any other DREW_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    drewgent gateway uninstall")
    print_info("    sudo drewgent gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Drewgent can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo drewgent gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo drewgent gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo drewgent gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.drewgent`` → ``ai.drewgent.gateway.plist`` (backward compatible).
    Profile ``~/.drewgent/profiles/coder`` → ``ai.drewgent.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.drewgent.gateway-{suffix}" if suffix else "ai.drewgent.gateway"
...
def get_drewgent_cli_path() -> str:
    """Get the path to the drewgent CLI."""
    ...
    return f"{get_python_path()} -m drewgent_cli.main"
...
def _drewgent_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current DREW_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_drewgent_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.hermes                    → /home/alice/.hermes
      /root/.hermes/profiles/coder     → /home/alice/.hermes/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes = get_drewgent_home().resolve()
    current_default = (Path.home() / ".drewgent").resolve()
    target_default = Path(target_home_dir) / ".hermes"

    # Default ~/.drewgent → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.drewgent → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.drewgent) — keep as-is
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        drewgent_home = _drewgent_home_for_target_user(home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m drewgent_cli.main gateway run --replace
...
Environment="DREW_HOME={drewgent_home}"
Restart=on-failure
RestartSec=30
KillMode=mixed
KillSignal=SIGTERM
...
"""
    ...
    drewgent_home = str(get_drewgent_home().resolve())
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Drewgent install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}drewgent gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}drewgent gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}drewgent gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}drewgent gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}drewgent gateway start{scope_flag}")
...
def generate_launchd_plist() -> str:
    ...
    working_dir = str(PROJECT_ROOT)
    drewgent_home = str(get_drewgent_home().resolve())
    log_dir = get_drewgent_home() / "logs"
...
<dict>
    ...
    <array>
        ...
        <string>-m</string>
        <string>drewgent_cli.main</string>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Drewgent install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  drewgent gateway status             # Check status")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Drewgent install")
    else:
        print("⚠ Service definition is stale relative to the current Drewgent install")
        print("  Run: drewgent gateway start")
    ...
    if deep:
        log_file = get_drewgent_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Drewgent Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Drewgent will log in directly",
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Drewgent delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Drewgent agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Drewgent will use (e.g., hermes@gmail.com)."},
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_drewgent_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'drewgent pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: drewgent pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'drewgent gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from drewgent_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"DrewgentAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: drewgent gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: drewgent gateway")
    else:
        print()
        print_info("No platforms configured. Run 'drewgent gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: drewgent gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if is_linux():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    drewgent gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: drewgent gateway start")
                sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  drewgent gateway install")
                print("  sudo drewgent gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  drewgent gateway          # Run in foreground")
                print("  drewgent gateway install  # Install as user service")
                print("  sudo drewgent gateway install --system  # Install as boot-time system service")
