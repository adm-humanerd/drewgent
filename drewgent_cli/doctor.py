"""
Doctor command for drewgent CLI.

Diagnoses issues with Drewgent Agent setup.
"""
...
_DHH = display_drewgent_home()  # user-facing display path (e.g. ~/.drewgent or ~/.drewgent/profiles/coder)

# Load environment variables from ~/.drewgent/.env so API key checks work
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.drewgent/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Drewgent Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.drewgent/.env (primary location for user config)
    env_path = DREW_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'drewgent setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'drewgent setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'drewgent setup' to create one")
                issues.append("Run 'drewgent setup' to create .env")
    
    # Check ~/.drewgent/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        try:
            from drewgent_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'drewgent setup' to migrate config")
                else:
                    issues.append("Run 'drewgent doctor --fix' or 'drewgent setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'drewgent doctor --fix'")
        except Exception:
            pass

        # Validate config structure (catches malformed custom_providers, etc.)
        try:
            from drewgent_cli.config import validate_config_structure
    ...
    try:
        from drewgent_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    drewgent_home = DREW_HOME
    if drewgent_home.exists():
    ...
    for subdir_name in expected_subdirs:
        subdir_path = drewgent_home / subdir_name
    ...
    soul_path = drewgent_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Drewgent a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Drewgent Agent Persona\n\n"
                "<!-- Edit this file to customize how Drewgent communicates. -->\n\n"
                "You are Drewgent, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = drewgent_home / "memories"
    ...
    state_db_path = drewgent_home / "state.db"
    ...
    wal_path = drewgent_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'drewgent doctor --fix' to checkpoint")
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint — https://github.com/NousResearch/drewgent-agent/issues/811
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'drewgent setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: drewgent skills list)")
    ...
    try:
        ...
        if not _honcho_cfg_path.exists():
            check_warn("Honcho config not found", "run: drewgent memory setup")
        elif not hcfg.enabled:
            check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
        elif not (hcfg.api_key or hcfg.base_url):
            check_fail("Honcho API key or base URL not set", "run: drewgent memory setup")
            issues.append("No Honcho API key — run 'drewgent memory setup'")
        else:
    ...
    try:
        ...
        if mem0_key:
            ...
            check_warn("Mem0 not configured", "(set MEM0_API_KEY in .env or run drewgent memory setup)")
    except ImportError:
    ...
    # =========================================================================
    try:
        from drewgent_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'drewgent doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
