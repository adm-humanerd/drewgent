"""
Interactive setup wizard for Drewgent Agent.
...
from drewgent_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://drewgent-agent.nousresearch.com/docs"
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from drewgent_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    from drewgent_cli.config import get_env_value
...
# display_drewgent_home imported lazily at call sites (stale-module safety during drewgent update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Drewgent Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Drewgent using environment variables or config commands:")
    print_info("  drewgent config set model.provider custom")
    print_info("  drewgent config set model.base_url http://localhost:8080/v1")
    print_info("  drewgent config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'drewgent setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from drewgent_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'drewgent setup')")


def _print_setup_summary(config: dict, drewgent_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'drewgent setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'drewgent setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'drewgent setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'drewgent setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {drewgent_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('drewgent setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('drewgent setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('drewgent setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('drewgent setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('drewgent setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('drewgent config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('drewgent config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('drewgent config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('drewgent', Colors.GREEN)}              Start chatting")
    print(f"   {color('drewgent gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('drewgent doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `drewgent tools` and `drewgent setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``drewgent model``)
    ...
    provider added to ``drewgent model`` is automatically available here.
    ...
    # Delegate to the shared drewgent model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from drewgent_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: drewgent model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Drewgent can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Drewgent can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'drewgent setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.drewgent/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'drewgent setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Drewgent runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Drewgent via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("DREW_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `drewgent setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("DREW_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("DREW_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Drewgent delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://drewgent-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Drewgent delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Drewgent delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'drewgent whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'drewgent whatsapp' to choose your mode (separate bot number")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://drewgent-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://drewgent-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  drewgent config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Drewgent from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'drewgent setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     drewgent config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: drewgent gateway install")
            else:
                print_info("  You can install later: drewgent gateway install")
                if _is_linux:
                    print_info("  Or as a boot-time service: sudo drewgent gateway install --system")
                print_info("  Or run in foreground:  drewgent gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   drewgent gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `drewgent setup tools` and `drewgent tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from drewgent_cli.config
...
def _offer_openclaw_migration(drewgent_home: Path) -> bool:
    ...
    print_info("Drewgent can import your settings, memories, skills, and API keys.")
    print()
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "openclaw_to_drewgent", _OPENCLAW_SCRIPT
        )
        ...
        migrator = mod.Migrator(
            ...
            target_root=drewgent_home.resolve(),
            execute=True,
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Drewgent.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      drewgent setup           — full or quick (auto-detected)
      drewgent setup model     — just model/provider
      drewgent setup terminal  — just terminal backend
      drewgent setup gateway   — just messaging platforms
      drewgent setup tools     — just tool configuration
      drewgent setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Drewgent Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Drewgent Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Drewgent Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Drewgent configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, drewgent_home)
            return
        elif choice == 1:
            ...
            print_info("Exiting. Run 'drewgent setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'drewgent setup' again when ready.")
            return
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, drewgent_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(drewgent_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Drewgent?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, drewgent_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {drewgent_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'drewgent config edit'")
...
def _offer_launch_chat():
    ...
    print()
    if prompt_yes_no("Launch drewgent chat now?", True):
        from drewgent_cli.main import cmd_chat
        from types import SimpleNamespace
...
def _run_first_time_quick_setup(config: dict, drewgent_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'drewgent setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    drewgent setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  drewgent setup gateway")
    print()
...
def _run_quick_setup(config: dict, drewgent_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'drewgent setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Drewgent to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'drewgent setup gateway'.")
